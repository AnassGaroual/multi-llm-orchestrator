 ...
 *
 * <p>Requires application.properties: spring.mvc.throw-exception-if-no-handler-found=true
 * spring.web.resources.add-mappings=false app.http.correlation-header=X-Correlation-Id
 */
@SpringBootTest(
...
@Import({
  ...
  com.multi.adapters.infra.http.CorrelationIdFilter.class,
...
class ProblemDetailsHandlerTest {
  ...
  @Test
  @DisplayName(
      "400 validation: body fails @NotBlank → RFC9457 problem with errors[] and correlation")
  void validation_error_returns_problem_details() throws Exception {
    var body = json();

    mvc.perform(post("/t/echo").contentType(MediaType.APPLICATION_JSON).content(body))
  ...
  void unsupported_media_type_returns_problem_details() throws Exception {
    mvc.perform(post("/t/echo").contentType(MediaType.TEXT_PLAIN).content("name=foo"))
